...
function checkJogo(objeto: unknown): objeto is Jogo {
...
async function handleData() {
  // O await está falando para esperar as respostas das requisições para depois armazenar o retorno na variável.
  const jogo = (await fetchData("jogo.json")) as Jogo; // Executa a função fetchData passando a url da api como parâmetro e definindo o tipo da variável T como a interface Jogo e armazena o retorno na variável jogo.
  ...
  const livro = (await fetchData("livro.json")) as Livro; // Executa a função fetchData passando a url da api como parâmetro e definindo o tipo da variável T como a interface Livro e armazena o retorno na variável livro.
  console.log(livro);
}
